...
/* 자바 파일이 여러개, form 연결이 다수일 경우 / 위치로 연결해주는 것 */
public class ExampleServlet2 extends HttpServlet {

	// Get 방식 요청을 (method="get"인 form 태그 제출) 처리하는 메서드
	@Override // HttpServlet 에서 가져와서 개발자에 맞게 다시 작성해주는 것이기 때문에 Override 작성
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		*/
		...
		// <input type="text" name="inputName"> 여기에 작성되어 제출된 값 얻어오기
		String inputName = request.getParameter("inputName");
		// --> 고객들이 작성한 값을 getParameter에서 name에 지정한 별칭으로 가져오기
		// <input type="number" name="inputAge"> 여기에 작성되어 제출된 값 얻어오기
		...
		/*
			http 
			404 : 웹 페이지 찾을 수 없을 때 발생하는 에러 코드
			-> 프로젝트 위치나 이름이 다를 때 [경로]
			-> index.html ExampleServlet2가 서로 파일 위치 찾지 못해서 발생하는 에러
			-> springboot 에서는 이런 에러 발생하지 않음
		*/
